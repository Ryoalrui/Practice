...
require_once './lib/Page.php';//包含文件：分页展示类
ini_set('display_errors', 0);

//连接数据库，设置字符集，查询数据库内的相关信息，将其显示在前端页面
$mysqli = new mysqli('localhost', 'root', '1010LING', 'hw');
$mysqli->query('set names utf8');
$fin = $mysqli->query('SELECT COUNT(*) AS total FROM pic_upload_system');
$number = 9;//每一页显示9条数据
$total = $fin->fetch_all(MYSQLI_ASSOC);//数据库内总数据数
$Page = new Page($number, $total[0]['total']);//实例化分页展示类
$links = $Page->allUrl();//获取页码链接
$data = $mysqli->query('SELECT info,addr FROM pic_upload_system LIMIT '.$Page->limit());
    ...
    <div class="navbar navbar-inverse navbar-fixed-top">
        <div class="container">
            <div class="navbar-header">
                ...
                <a class="navbar-brand hidden-sm" href="index.php" onclick="_hmt.push(['_trackEvent', 'navbar', 'click', 'navbar-首页'])">酷图</a>
            </div>
    ...
    <div class="container projects">
        ...
        <nav aria-label="Page navigation" style="text-align:center">
            <ul class="pagination pagination-lg">
                <li>
                    <a href="<?php echo $links['first']; ?>" aria-label="Previous">
                        <span aria-hidden="true">首页</span>
                    </a>
                </li>
                <?php for ($i=1;$i<=ceil($total[0]['total']/$number);$i++) :
                      if ($Page->page==$i) {
                          echo '<li><a style="color:red">'.$i.'</a></li>';
                      } else {
                          echo '<li><a href="'.$_SERVER['PHP_SELF'].'?page='.$i.'">'.$i.'</a></li>';
                      }
                endfor;?>
                <li>
                    <a href="<?php echo $links['end']; ?>" aria-label="Next">
